    ...
    export default {
        ...
        methods: {
            ...
            modalOpened: function () {
                ...
                // zaznaczenie kolor�w zale�nie od aktualnych ustawie�
                ...
                settingsNodeColor.addEventListener("click", function (event) {
                    ...
                    if (event.target.id != '') {
                        ...
                        for (var i = 0; i < clickedElement.length; i++) {     // usuwa zaznaczenie ze wszysztkich kolor�w
            ...
            saveSettings: function () {
                ...
                this.changeColor([this.cy.nodes(), this.nodesDefaultColor, this.cy.nodes().classes()])    // zmiana kolor�w node'�w
                this.changeColor([this.cy.edges(), this.edgesDefaultColor, this.cy.edges().classes()])    // zmiana kolor�w edge'�w
            ...
            independentSetClicked: function (a) {
                ...
                if (a.path === undefined) {
                    ...
                    indexOfDot = cleanString.indexOf('.');   // musze sprawdzac od kropki, bo inaczej np zbi�r "15" bra� jako "1"
                ...
                if (!(a.path === undefined)) {
                    ...
                    indexOfDot = textOfClickedSet.indexOf('.');   // musze sprawdzac od kropki, bo inaczej np zbi�r "15" bra� jako "1"
                    setIndex = textOfClickedSet.substring(0, indexOfDot) - 1;

                    //console.log(setIndex);
                }

                // var setIndex = a.path[0].innerText.charAt(0) - 1;   // -1, �eby na stronie mog�y si� zaczyna� od 1, a nie od 0
            ...
            view_init: function () {
                ...
                // menu wierzcho�k�w lub kraw�dzi
                this.cy.cxtmenu({
                    selector: 'node',
                    commands: [
                        {
            ...
            calculateIndependentNodeSets: function () {
                ...
                var i = 0;
                var j = 0;
                var minimumVertexCoverSets = [];      // stare nazwy, zmieni� na independent node sets
            ...
            calculateIndependentEdgeSets: function () {
                ...
                for (i = 0; i < nodes.length; i++) {
                    ...
                    for (j = 0; j < nodeEdges.length; j++) {
                        edgesForSingleNode.push(nodeEdges[j]._private.data.id);   // w nawiasach maj� by� kombinacje kraw�dzi.... nice
                ...
                var i = 0;
                var j = 0;
                var maximumIndependentEdgeSets = [];      // stare nazwy, zmieni� na independent node sets
            ...
            calculateMinimumVertexCover: function () {
                ...
                var p = [];
                // zamiana id element�w na ich label, czyli name
                var o;
                var u;
            ...
            loadGraphTxt: function () {
                ...
                var textFromFileLoaded = '';
                this.i = 0;
                this.cy.elements().remove();    //usuni�cie aktualnego grafu
                ...
                fileReader.onload = function (fileLoadedEvent) {
                    ...
                    var rowsSplitted = rowsSplittedRaw.filter(function (el) {   // usuni�cie pustych wierszy
                        return el != "";
                    });
                    ...
                    for (var i = 0; i < rowsSplitted.length; i++) {   // przechodzi przez kolejne wiersze pliku
                        ...
                        for (var j = 0; j < columnsSplitted.length; j++) {    //przechodzi przez kolejne kolumny wiersza
                            if (i == 0) {   // pierwszy wiersz to nazwy wierzcho�k�w
                            ...
                            } else if (i > 0) {         // od drugiego wiersza s� po��czenia mi�dzy node'ami

                                if (j > (i - 1)) {        // �eby nie tworzy�o kraw�dzi dwa razy (macierz jest symetryczna)
                ...
                for (var i = 0; i < (nodes.length + 1); i++) {  // kolejne wiersze stringa
                    ...
                    for (var j = 0; j < nodes.length; j++) {  // kolejne nody danego wiersza
                        ...
                        if (i == 0) {   // pierwszy wiersz to nazwy wierzcho�k�w
                            ...
                            //if(nodes[j].) // je�eli node [j] jest po��czony z nodem [i-1] -> daj jedynk�
                            var isConnected = false;
                            var edgeName = '';
                            for (var k = 0; k < nodes[j]._private.edges.length; k++) {   // sprawdza kraw�dzie po��czone z [j], czy nie s� po��czone tak�e z [i]
                                var tempEdge = nodes[j]._private.edges[k]._private.data;
                                if (tempEdge.target == nodes[i - 1]._private.data.id) {    // zamiast 1 kiedy jest po��czenie, b�dzie nazwa kraw�dzi
                                    isConnected = true;
                    ...
                    if (i == 0) {
                        endString += '\n';  // dodatkowy enter po wierszu z nazwami wierzcho�k�w
                    }
                    endString += '\n';
                }
        ...
        mounted: function () {
            ...
            let routeGraphData = this.$route.params.graphData;

            if (routeGraphData != undefined)
            {
                this.cy.json(JSON.parse(routeGraphData));
                this.i = this.cy.elements().length + 1;
                this.cy.fit();
            }
            ...
            */
            ...
            /*
            // WARNING. Tutaj zawsze ustalam widoczno�� wszystkich element�w. USU� POTEM!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            this.cy.nodes().removeClass('hide-label');
            this.cy.edges().addClass('show-label');
            */
        }
